...
const DeleteContactModal = ({ id, onClose, getContactMessages }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await axios.post("/api/deleteMessages", data);
    ...
    <Modal open onClose={onClose} disableScrollLock>
      ...
      >
        ...
        <Typography sx={{ marginBottom: 4,color:"white" }}>
          ...
          {id.length === 1 ? "Message" : "Messages"} ? This action cannot be
          undone.
        </Typography>
        <Box
          sx={{
